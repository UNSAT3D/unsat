...
from data import DataSelection, XRayDataset, create_dataloaders
...
def test_dataset(create_test_h5, remove_test_h5):
    ...
    with h5py.File(create_test_h5, 'r') as hdf5_file:
        np.testing.assert_equal(dataset[0][0].numpy(), hdf5_file['coarse/loose/04']['data'][0, 5])
        # second day
        np.testing.assert_equal(dataset[5][0].numpy(), hdf5_file['coarse/loose/04']['data'][1, 5])
        # second sample
        np.testing.assert_equal(dataset[10][0].numpy(), hdf5_file['fine/dense/07']['data'][0, 5])
